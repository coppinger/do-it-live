...
<div class={invisible ? 'max-h-[0px]' : ''} aria-hidden={invisible}>
	...
	>
		...
		{:else if type === 'subscribe'}
			<span class="order-2 font-bold">{username}</span>
			<span class="order-3 font-medium text-slate-400">•</span>
			<span class="order-3 font-medium text-slate-400">subscribed to you</span>
		{:else if type === 'raid'}
			<span class="order-2 font-bold">{username}</span>
			<span class="order-3 font-medium text-slate-400">•</span>
			<span class="order-3 font-medium text-slate-400">gave you a shoutout to 317 viewers</span>
		{:else if type === 'shoutout'}
			<span class="order-2 font-bold">{username}</span>
